...
export async function getResources(): Promise<Resource[]> {
  try {
    ...
    // Si aucune ressource n'est trouvée dans localStorage, essayer Supabase
...
export async function addResource(title: string, url: string, type: string): Promise<Resource | null> {
  try {
    ...
    // Créer la nouvelle ressource sans ID (Supabase générera l'UUID)
    const newResource = {
      title,
      url,
      type,
    ...
    try {
      ...
      if (error) {
        ...
        // Fallback à localStorage si Supabase échoue
      } else if (data) {
        // Mettre à jour localStorage avec les données de Supabase
        try {
    ...
    // Fallback: Ajouter à localStorage si Supabase a échoué
...
export async function deleteResource(resourceId: string): Promise<boolean> {
  try {
    ...
    // Essayer de supprimer de Supabase si ce n'est pas un ID local
    if (!resourceId.startsWith("local-")) {
      try {
        ...
        if (error) {
          ...
          // Mettre à jour les ordres dans Supabase pour les ressources non locales
...
export async function moveResource(resourceId: string, direction: "up" | "down"): Promise<boolean> {
  try {
    ...
    // Trouver l'index de la ressource à déplacer
    ...
    // Mettre à jour Supabase pour les ressources non locales
