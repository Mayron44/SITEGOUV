...
export default function CreateNewsletterPage() {
  ...
  const saveNewsletter = async () => {
    ...
    try {
      const newNewsletter = await createNewsletter(title, content, image || undefined)

      if (newNewsletter) {
        toast({
          title: "Newsletter sauvegardée",
          description: "La newsletter a été sauvegardée avec succès.",
        })

        // Rediriger vers la page de gestion des newsletters
        router.push("/intranet/newsletter")
      } else {
        throw new Error("Échec de la création de la newsletter")
      }
    } catch (error) {
      console.error("Error saving newsletter:", error)
      toast({
        title: "Erreur",
        description: "Une erreur est survenue lors de la sauvegarde de la newsletter.",
        variant: "destructive",
      })
    }
  }
    ...
    <div className="container py-8">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-3xl font-bold mb-2">Créer une newsletter</h1>
            <p className="text-gray-500">Rédigez une nouvelle newsletter à envoyer aux abonnés</p>
          </div>
        </div>
